...
from dfrobot_huskylensv2_mqtt import (
    ...
    ALGORITHM_OCR_RECOGNITION,
    ALGORITHM_LINE_TRACKING,
    ALGORITHM_EMOTION_RECOGNITION,
    ALGORITHM_TAG_RECOGNITION,
    ALGORITHM_QRCODE_RECOGNITION,
...
def main():
    """主函数"""
    # 使用配置文件中的默认值，如果配置文件不存在则使用硬编码的默认值
    parser = argparse.ArgumentParser(description='二哈识图V2 MQTT物体识别测试')
    ...
    parser.add_argument('--algorithm', type=int, default=ALGORITHM_OBJECT_RECOGNITION,
                        help=f'要测试的算法ID (默认: {ALGORITHM_OBJECT_RECOGNITION} - 物体识别)')
    ...
    print(f"  测试算法ID: {args.algorithm} (物体识别)")
    print()
